...
import {
  Play, Pause, Radio, Newspaper, Calendar, Gift, Podcast, Video,
  MessageSquare, Search, ChevronRight, Heart, Share2, Volume2,
  Zap, TrendingUp, Clock, Sparkles, Instagram, Facebook, Youtube,
  Menu, X, ArrowRight, Headphones, Mic2,
} from "lucide-react";
...
type Programa = {
  ...
  imageUrl?: string | null; startTime: string; endTime: string;
...
type Config = {
  streamUrl: string; streamName: string; isLive: boolean;
  nowPlayingTitle: string | null; nowPlayingArtist: string | null;
};
      ...
      <header className="fixed top-0 left-0 right-0 z-50 glass-dark border-b border-white/5">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <Link href="/app" prefetch className="px-4 py-2 text-sm font-medium text-white/70 hover:text-white transition rounded-full hover:bg-white/5">Início</Link>
            <Link href="/app/radio" prefetch className="px-4 py-2 text-sm font-medium text-white/70 hover:text-white transition rounded-full hover:bg-white/5">Rádio</Link>
            <Link href="/app/noticias" prefetch className="px-4 py-2 text-sm font-medium text-white/70 hover:text-white transition rounded-full hover:bg-white/5">Notícias</Link>
            <Link href="/app/programacao" prefetch className="px-4 py-2 text-sm font-medium text-white/70 hover:text-white transition rounded-full hover:bg-white/5">Programação</Link>
            <Link href="/app/promocoes" prefetch className="px-4 py-2 text-sm font-medium text-white/70 hover:text-white transition rounded-full hover:bg-white/5">Promoções</Link>
            <Link href="/admin" className="px-4 py-2 text-sm font-medium text-white/70 hover:text-white transition rounded-full hover:bg-white/5">Admin</Link>
          </nav>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 py-20 text-center">
          ...
          <div className="mx-auto mb-8 animate-splash-scale animate-splash-glow">
            <Image
              ...
              height={274}
              priority
              className="mx-auto w-72 sm:w-96 h-auto object-contain"
            />
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1 text-left">
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white truncate">
                  {mounted && nowPlaying?.title ? nowPlaying.title : "Flash Mix Digital"}
                </h3>
          ...
          <div className="flex flex-wrap justify-center gap-3 animate-fade-in-up">
            <Link href="/app" prefetch className="inline-flex items-center gap-2 rounded-full bg-white text-[#090909] px-6 py-3 text-sm font-bold uppercase tracking-wider hover:scale-105 active:scale-95 transition">
              <Headphones className="h-4 w-4" /> Abrir o App
            </Link>
            <Link href="/app/programacao" prefetch className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 backdrop-blur px-6 py-3 text-sm font-bold uppercase tracking-wider text-white hover:bg-white/10 transition">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              const Icon = q.icon;
              return (
                <Link key={q.href} href={q.href} prefetch className="group glass rounded-3xl p-5 transition hover:scale-105 active:scale-95">
        ...
        <section className="py-16 px-4 sm:px-6 bg-gradient-to-b from-transparent via-[#0B1836]/30 to-transparent">
          <div className="mx-auto max-w-7xl">
            <div className="flex items-end justify-between mb-8">
              ...
              <Link href="/app/programacao" prefetch className="hidden sm:inline-flex items-center gap-1 text-sm font-bold uppercase tracking-wider text-[#E30613] hover:text-[#ff1f2c]">
        ...
        <section className="py-16 px-4 sm:px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {promocoes.map((p) => (
                <Link key={p.id} href={`/app/promocoes/${p.id}`} prefetch className="group relative overflow-hidden rounded-3xl border border-white/10 transition hover:scale-[1.02] active:scale-95">
        ...
        <section className="py-16 px-4 sm:px-6 bg-gradient-to-b from-transparent via-[#0B1836]/30 to-transparent">
          <div className="mx-auto max-w-7xl">
            <div className="flex items-end justify-between mb-8">
              ...
              <Link href="/app/noticias" prefetch className="hidden sm:inline-flex items-center gap-1 text-sm font-bold uppercase tracking-wider text-[#E30613] hover:text-[#ff1f2c]">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {noticias.slice(0, 3).map((n) => (
                <Link key={n.id} href={`/app/noticias/${n.id}`} prefetch className="group glass rounded-3xl overflow-hidden transition hover:scale-[1.02] active:scale-95">
                  {n.imageUrl && (
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-xs text-white/40 mt-3">
                      ...
                      {new Date(n.publishedAt).toLocaleDateString("pt-BR", { day: "2-digit", month: "long" })}
                    </p>
                  </div>
                </Link>
              ))}
          ...
          <div className="glass-red rounded-[2rem] p-8 sm:p-12 text-center relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap justify-center gap-3">
                ...
                <Link href="/app" prefetch className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 backdrop-blur px-7 py-3.5 text-sm font-bold uppercase tracking-wider text-white hover:bg-white/10 transition">
      ...
      <footer className="border-t border-white/5 py-10 px-4 sm:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                <li><Link href="/app/radio" prefetch className="text-white/60 hover:text-white transition">Rádio ao Vivo</Link></li>
                <li><Link href="/app/noticias" prefetch className="text-white/60 hover:text-white transition">Notícias</Link></li>
                <li><Link href="/app/programacao" prefetch className="text-white/60 hover:text-white transition">Programação</Link></li>
                <li><Link href="/app/promocoes" prefetch className="text-white/60 hover:text-white transition">Promoções</Link></li>
              </ul>
            </div>
          ...
          <div className="mt-8 pt-6 border-t border-white/5 text-center text-xs text-white/40">
            ...
            <Link href="/admin" className="hover:text-white ml-1">Admin</Link>
          </div>
        </div>
      </footer>
    </div>
