...
const db = new PrismaClient({
  datasources: {
    db: { url: 'postgresql://neondb_owner:npg_FM85HkUCXpzu@ep-holy-glade-atms4z5e-pooler.c-9.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require&pgbouncer=true&connect_timeout=15' }
  }
});
...
async function main() {
  ...
  const streamUrl = "http://s02.taaqui.org:8874/stream";
  ...
  // Confirmação final com counts
  const finalCounts = {
    locutores: await db.locutor.count(),
    programas: await db.programa.count(),
    musicas: await db.musica.count(),
    noticias: await db.noticia.count(),
    promocoes: await db.promocao.count(),
    ...
    radioConfig: await db.radioConfig.count(),
