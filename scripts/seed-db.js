// Script para popular o banco Neon diretamente (sem passar pelo Next.js)
...
async function main() {
  ...
  for (const l of locutoresData) {
    ...
    if (!exists) { await db.locutor.create({ data: l }); count++; }
  }
  ...
  for (const p of programasData) {
    const exists = await db.programa.findFirst({ where: { title: p.title, dayOfWeek: p.dayOfWeek } });
    if (!exists) { await db.programa.create({ data: p }); count++; }
  }
  ...
  // Notícias
  const noticiasData = [
    { title: "Flash Mix Digital lança novo aplicativo mobile", summary: "Rádio inova com plataforma digital premium dark mode neon.", content: "A Flash Mix Digital acaba de lançar seu novo aplicativo mobile com design premium, dark mode e neon. A plataforma traz player em segundo plano, recados dos ouvintes e programação completa.", imageUrl: "https://picsum.photos/seed/news1/600/400", category: "Tecnologia", isFeatured: true },
    { title: "Campo Grande sedia festival de música neste fim de semana", summary: "Evento ocorre no próximo final de semana com entrada gratuita.", content: "A prefeitura municipal de Campo Grande anunciou a realização de um grande festival cultural que vai movimentar a cidade.", imageUrl: "https://picsum.photos/seed/news2/600/400", category: "Cultura", isFeatured: true },
    { title: "Promoção Verão Flash Mix tem prêmios de R$ 10 mil", summary: "Participe e concorra a prêmios incríveis.", content: "A promoção Verão Flash Mix está sorteando R$ 10 mil em prêmios para os ouvintes.", imageUrl: "https://picsum.photos/seed/news3/600/400", category: "Promoção", isFeatured: false },
    { title: "Entrevista exclusiva com artista local", summary: "Bate-papo com quem está fazendo sucesso.", content: "No programa Conectados com Você desta sexta-feira teremos uma entrevista exclusiva.", imageUrl: "https://picsum.photos/seed/news4/600/400", category: "Música", isFeatured: false },
    { title: "Top 10 mais tocadas da semana", summary: "Confira as músicas que bombaram.", content: "A Flash Mix Digital divulga o Top 10 das músicas mais tocadas da semana.", imageUrl: "https://picsum.photos/seed/news5/600/400", category: "Música", isFeatured: false },
  ];
  count = 0;
  for (const n of noticiasData) {
    const exists = await db.noticia.findFirst({ where: { title: n.title } });
    if (!exists) { await db.noticia.create({ data: n }); count++; }
  }
  results.noticias = count;

  // Promoções
  const promocoesData = [
    { title: "Verão Flash Mix", description: "Concorra a R$ 10 mil em prêmios!", rules: "Para participar, envie um recado pelo app ou WhatsApp. Sorteio ao vivo no programa Manhã Flash.", prize: "R$ 10.000,00", imageUrl: "https://picsum.photos/seed/promo1/600/400", endDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), isActive: true, participants: 1247 },
    { title: "Ingressos VIP Festival", description: "Venha para o nosso evento exclusivo!", rules: "Os 50 primeiros ouvintes que enviarem recado ganham.", prize: "Ingressos VIP", imageUrl: "https://picsum.photos/seed/promo2/600/400", endDate: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000), isActive: true, participants: 432 },
    { title: "Flash Back Friday", description: "Sexta-feira de clássicos com prêmios!", rules: "Toda sexta, sorteio de kit Flash Mix.", prize: "Kit Flash Mix", imageUrl: "https://picsum.photos/seed/promo3/600/400", endDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), isActive: true, participants: 89 },
  ];
  count = 0;
  for (const p of promocoesData) {
    const exists = await db.promocao.findFirst({ where: { title: p.title } });
    if (!exists) { await db.promocao.create({ data: p }); count++; }
  }
  ...
  const podcastsData = [
    ...
    { title: "Bate-Papo com DJ Flash", description: "Entrevistas com artistas locais.", imageUrl: "https://picsum.photos/seed/pod2/600/400", audioUrl: "https://example.com/pod2.mp3", duration: 2400, category: "Música", episode: 1, season: 1 },
    { title: "Hora do Rock - Especial", description: "Histórias das maiores bandas do rock.", imageUrl: "https://picsum.photos/seed/pod3/600/400", audioUrl: "https://example.com/pod3.mp3", duration: 3600, category: "Rock", episode: 1, season: 1 },
    { title: "Sertanejo na Veia", description: "Tudo sobre o universo sertanejo.", imageUrl: "https://picsum.photos/seed/pod4/600/400", audioUrl: "https://example.com/pod4.mp3", duration: 2700, category: "Sertanejo", episode: 1, season: 1 },
  ];
  count = 0;
  for (const p of podcastsData) {
    const exists = await db.podcast.findFirst({ where: { title: p.title } });
    if (!exists) { await db.podcast.create({ data: p }); count++; }
  }
  ...
  for (const v of videosData) {
    ...
    if (!exists) { await db.video.create({ data: v }); count++; }
  }
  ...
  for (const b of bannersData) {
    ...
    if (!exists) { await db.banner.create({ data: b }); count++; }
  }
  ...
  for (const c of categoriasData) {
    ...
    if (!exists) { await db.categoria.create({ data: c }); count++; }
  }
  ...
  if (!configExists) {
    ...
    if (configExists.streamUrl !== streamUrl) {
      await db.radioConfig.update({ where: { id: configExists.id }, data: { streamUrl } });
    }
  ...
  if (!adminExists) {
    ...
    await db.user.create({
      data: { email: adminEmail, name: "Administrador", passwordHash, role: "ADMIN", isGuest: false, city: "Campo Grande", state: "MS" },
    });
    results.adminUser = 1;
  } else {
    results.adminUser = 0;
  ...
  for (const n of notifsData) {
    ...
    if (!exists) { await db.notification.create({ data: n }); count++; }
  }
